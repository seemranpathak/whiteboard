...
// import { Canvas as FabricCanvas, Circle, Rect, PencilBrush, BaseBrush } from "fabric";
...
// // Simple debounce utility function to optimize event handling (e.g., window resize)
...
//   const canvasRef = useRef<HTMLCanvasElement>(null); // Ref for the HTML canvas element
//   const [fabricCanvas, setFabricCanvas] = useState<FabricCanvas | null>(null); // State to hold the Fabric.js canvas instance
//   const [activeColor, setActiveColor] = useState("#8B5CF6"); // State for the active drawing color (default purple)
//   const [activeTool, setActiveTool] = useState<DrawingTool>("select"); // State for the currently active tool (default 'select')
//   const [brushSize, setBrushSize] = useState(2); // State for the drawing brush size

// const [socket, setSocket] = useState<Socket | null>(null);

//   // Mock collaborators for demonstration purposes.
//   // In a real app, this would come from a Socket.IO connection.
//   const [collaborators] = useState([
//     { id: "1", name: "You", color: "#8B5CF6", isActive: true },
//     // { id: "2", name: "Alice", color: "#10B981", isActive: true },
//     // { id: "3", name: "Bob", color: "#EF4444", isActive: true },
//   ]);
...
// //     // Update cursorPositions state for UserCursors component
// //   });

// //   // Add Fabric.js event listeners to emit changes
// //   if (fabricCanvas) {
// //     fabricCanvas.on('object:modified', () => {
// //       // Emit canvas state or modified object data
// //       // socket?.emit('objectModified', { canvasState: fabricCanvas.toJSON() });
// //     });
// //     fabricCanvas.on('path:created', (e) => {
// //       // Emit the new path data
// //       // socket?.emit('drawing', { path: e.path?.toJSON() });
// //     });
// //     // ... other Fabric.js events
// //   }

// //   return () => {
// //     newSocket.disconnect();
// //   };
// // }, [fabricCanvas]); // Re-run when fabricCanvas is ready
// //......................................................................
...
//   // useEffect hook for initializing the Fabric.js canvas
//   // This runs once when the component mounts.
//   useEffect(() => {
//     if (!canvasRef.current) return; // Ensure the canvas element is available

//     // Create a new Fabric.js canvas instance
//     const canvas = new FabricCanvas(canvasRef.current, {
//       width: window.innerWidth - 64, // Adjust width based on window size
//       height: window.innerHeight - 120, // Adjust height based on window size
//       backgroundColor: "white", // Set initial background color
//     });

//     // Initially set drawing mode to false (select mode)
//     canvas.isDrawingMode = false;

//     // Store the Fabric.js canvas instance in state
//     setFabricCanvas(canvas);
//     toast("Whiteboard ready! Start collaborating!"); // Show a toast notification
...
//     // Debounce the resize event listener for performance
//     const debouncedResize = debounce(handleResize, 200);
//     window.addEventListener("resize", debouncedResize);

//     // Cleanup function: This runs when the component unmounts
//     return () => {
//       canvas.dispose(); // Dispose of the Fabric.js canvas to prevent memory leaks
//       window.removeEventListener("resize", debouncedResize); // Remove the resize listener
//     };
//   }, []); // Empty dependency array means this effect runs only once

//   // useEffect hook for updating canvas properties based on active tool, color, and brush size
//   // This runs whenever these state variables change.
//   useEffect(() => {
//     if (!fabricCanvas) return; // Ensure canvas is initialized

//     let targetBrush: BaseBrush | undefined;

//     if (activeTool === "draw") {
//       fabricCanvas.isDrawingMode = true; // Enable drawing mode
//       // CRITICAL FIX: Ensure freeDrawingBrush is initialized.
//       // Fabric.js might lazy-initialize this, so we explicitly create a PencilBrush
//       // if it's not already set, preventing "possibly undefined" errors.
//       if (!fabricCanvas.freeDrawingBrush) {
//         fabricCanvas.freeDrawingBrush = new PencilBrush(fabricCanvas);
//         console.log("Fabric.js PencilBrush initialized for drawing mode.");
//       }
//       targetBrush = fabricCanvas.freeDrawingBrush;
//       targetBrush.color = activeColor; // Set brush color
//       targetBrush.width = brushSize; // Set brush width
//     } else if (activeTool === "eraser") {
//       fabricCanvas.isDrawingMode = true; // Eraser uses drawing mode with white color
//       // CRITICAL FIX: Ensure brush exists for eraser mode as well.
//       if (!fabricCanvas.freeDrawingBrush) {
//         fabricCanvas.freeDrawingBrush = new PencilBrush(fabricCanvas);
//         console.log("Fabric.js PencilBrush initialized for eraser mode.");
//       }
//       targetBrush = fabricCanvas.freeDrawingBrush;
//       targetBrush.color = "white"; // Eraser "color" is white
//       targetBrush.width = brushSize * 2; // Eraser is typically wider for better erasing
//     } else {
//       fabricCanvas.isDrawingMode = false; // Disable drawing mode for 'select' and shape tools
//     }
//   }, [activeTool, activeColor, brushSize, fabricCanvas]); // Dependencies for this effect

//   // Handler for when a tool button is clicked
//   const handleToolClick = (tool: DrawingTool) => {
//     setActiveTool(tool); // Update the active tool state

//     // Logic for adding shapes directly to the canvas
//     if (tool === "rectangle" && fabricCanvas) {
//       const rect = new Rect({
//         left: 100, // Initial position
//         top: 100,
//         fill: activeColor, // Fill color from active color
//         width: 100, // Default size
//         height: 100,
//         rx: 8, // Rounded corners
//         ry: 8,
//       });
//       fabricCanvas.add(rect); // Add rectangle to canvas
//       fabricCanvas.setActiveObject(rect); // Select the newly added object
//       fabricCanvas.renderAll(); // Re-render canvas to show the new object
...
//       });
//       fabricCanvas.add(circle); // Add circle to canvas
//       fabricCanvas.setActiveObject(circle); // Select the newly added object
//       fabricCanvas.renderAll(); // Re-render canvas to show the new object
//     }
//   };
...
//     fabricCanvas.clear(); // Clear all objects from the canvas
//     fabricCanvas.backgroundColor = "white"; // Ensure background remains white
//     fabricCanvas.renderAll(); // Re-render canvas
//     toast("Canvas cleared!"); // Show notification
//   };
...
//         {/* Mock User Cursors (excluding the current user) */}
//         <UserCursors collaborators={collaborators.filter(c => c.isActive && c.id !== "1")} />
//       </div>
//     </div>
//   );
// };
